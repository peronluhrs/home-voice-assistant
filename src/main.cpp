...
// JSON helper: string -> json or null
static inline nlohmann::json j_or_null(const std::string& s) {
...
static std::vector<DeviceInfo> listPaDevices() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        out.push_back({ i, di && di->name ? nlohmann::json(di->name) : nlohmann::json(""), hai && hai->name ? nlohmann::json(hai->name) : nlohmann::json("") });
    }
    Pa_Terminate();
    return out;
}
...
static int findDeviceIndex(const std::string& key, bool output) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        std::string nm = di->name ? nlohmann::json(di->name) : nlohmann::json("");
