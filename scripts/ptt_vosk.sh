...
REC_SEC="${REC_SEC:-3}"
MIC_DEV="${MIC_DEV:-default}"   # arecord -L pour la liste (default|pulse|hw:0,0|plughw:0,0)

rm -f "$FIFO"; mkfifo "$FIFO"

# lance l'assistant qui vocalise via espeak/pulse
"$REPO_DIR/scripts/assistant_talker.sh" --output-device pulse < "$FIFO" &
ASS_PID=$!
...
while true; do
  ...
  TEXT=$(python3 - <<'PY'
import json, wave, os, sys, re
try:
    from vosk import Model, KaldiRecognizer
except Exception:
    print("__VOSK_MISSING__"); sys.exit(0)
m = os.path.expanduser('~/models/vosk/vosk-model-small-fr-0.22')
if not os.path.isdir(m):
    print("__MODEL_MISSING__"); sys.exit(0)
...
# --- normalisation FR légère (corrige confusions fréquentes) ---
low = t.lower()
repls = {
  "capitaine de ": "capitale de ",
  "capitaine ": "capitale ",
  "capitaines": "capitales",
  "capitane": "capitale",
}
for k,v in repls.items():
    low = low.replace(k, v)

# remonte un texte propre (garde la casse simple)
print(low)
PY
)
