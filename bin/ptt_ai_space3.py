...
PIPER_MODEL = os.getenv("PIPER_MODEL")  # ex: /home/vincent/piper/fr_FR-...-medium.onnx
SPD_SAY = shutil.which("spd-say")

def say_fr(text: str):
    text = text.replace('"','\\"')
    # 1) Piper si dispo (et modèle fourni)
    if PIPER and PIPER_MODEL and os.path.exists(PIPER_MODEL):
        wav = "/tmp/piper_out.wav"
        cmd = f'{PIPER} --model {shlex.quote(PIPER_MODEL)} --output_file {shlex.quote(wav)} --text "{text}"'
        subprocess.run(cmd, shell=True)
        subprocess.run(f'paplay {shlex.quote(wav)}', shell=True)
        return
    # 2) RHVoice via speech-dispatcher si dispo
    if SPD_SAY:
        subprocess.run(f'spd-say -o rhvoice -l fr "{text}"', shell=True)
        return
    # 3) Fallback espeak-ng -> paplay (WAV)
    cmd = f'espeak-ng -v fr "{text}" --stdout | paplay'
    subprocess.run(cmd, shell=True)
...
def asr(path):
    ...
    m = Model(MODEL_DIR); rec = KaldiRecognizer(m, RATE)
...
if __name__=="__main__":
    say_fr("Espace prêt. Échap pour quitter.")
